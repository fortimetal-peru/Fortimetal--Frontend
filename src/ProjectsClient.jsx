...
export default function ProjectsClient() {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 640, margin: "0 auto" }}>
      ...
      <p style={{ color: "#9CA1A7", fontSize: 13.5, marginBottom: 16 }}>Sigue el avance de tus proyectos con FORTIMETAL.</p>
      ...
      {loading ? (
        ...
        <p style={{ color: "#9CA1A7", fontSize: 13.5 }}>Todavía no tienes proyectos asignados.</p>
      ) : (
            ...
            return (
              <button key={p.id} onClick={() => setOpenId(p.id)} style={row}>
                <div style={{ flex: 1, minWidth: 0, textAlign: "left" }}>
                  ...
                  <div style={{ marginTop: 6, height: 5, background: "#14171A", borderRadius: 999, overflow: "hidden" }}>
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 50, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#14171A", width: "100%", maxWidth: 640, maxHeight: "88vh", overflowY: "auto", borderRadius: "16px 16px 0 0", padding: 18 }}>
        ...
        {detail?.description && <p style={{ fontSize: 13, color: "#9CA1A7", marginBottom: 12 }}>{detail.description}</p>}
...
const row = { display: "flex", alignItems: "center", gap: 10, background: "#1C2126", border: "1px solid #2A2E33", borderRadius: 10, padding: "12px 12px", cursor: "pointer", width: "100%", textAlign: "left" };
