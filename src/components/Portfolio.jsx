        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {items.map((item) => (
            <button
              ...
              style={{ position: "relative", aspectRatio: "1", borderRadius: 12, overflow: "hidden", border: "1px solid #E5E6E3", padding: 0, cursor: "pointer", background: "#EDEEEC" }}
            >
              {item.photo_urls?.[0] && (
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, maxWidth: 480, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: 18 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
            <h2 className="fm-display" style={{ fontSize: 18, color: "#1A1A1A", margin: 0 }}>{item.title}</h2>
            <button onClick={onClose} style={{ background: "none", border: "none", color: "#4B5157", cursor: "pointer", padding: 4, flexShrink: 0 }}>
              <X size={20} />
            </button>
          </div>
          {item.description && (
            <p style={{ color: "#4B5157", fontSize: 13.5, marginTop: 8, lineHeight: 1.5 }}>{item.description}</p>
          )}
