            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 6 }}>
              {form.photo_urls.map((url) => (
                <div key={url} style={{ position: "relative", width: 64, height: 64 }}>
                  <img src={`${import.meta.env.VITE_API_BASE_URL || ""}${url}`} alt="" style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8, border: "1px solid #2A2E33" }} />
          ...
          {items.map((item) => (
            <div key={item.id} style={{ ...row, opacity: item.is_active ? 1 : 0.5 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#F4F4F3" }}>{item.title}</div>
...
const addPhotoBtn = { width: 64, height: 64, borderRadius: 8, border: "1px dashed #2A2E33", background: "#14171A", display: "flex", alignItems: "center", justifyContent: "center", color: "#9CA1A7", cursor: "pointer" };
